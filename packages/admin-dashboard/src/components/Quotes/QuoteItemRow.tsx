...
const QuoteItemRow: React.FC<QuoteItemRowProps> = ({
  ...
  const handleQuantityChange = (newQuantity: number) => {
    ...
    const updatedItem = {
      ...
      lineTotal: newQuantity * item.unitPrice * (1 + item.taxRate / 100),
    };
  ...
  const handleUnitPriceChange = (newPrice: number) => {
    ...
    const updatedItem = {
      ...item,
      unitPrice: newPrice,
      lineTotal: item.quantity * newPrice * (1 + item.taxRate / 100),
    };
    onUpdate(updatedItem);
  };
  ...
  const handleTaxRateChange = (newRate: number) => {
    ...
    const updatedItem = {
      ...item,
      taxRate: newRate,
      lineTotal: item.quantity * item.unitPrice * (1 + newRate / 100),
    };
    onUpdate(updatedItem);
  };
