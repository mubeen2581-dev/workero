...
import { mockInvoices, mockPayments } from '@/mocks/invoices';
...
const InvoiceList: React.FC<InvoiceListProps> = ({
  ...
  const handleBulkAction = (action: string) => {
    if (action === 'send_payment_links') {
      selectedInvoices.forEach(async (id) => {
        await XEPayService.createPaymentLink({ invoiceId: id, amount: 0, currency: 'GBP', description: 'Payment', customerEmail: '' });
      });
      console.log('Payment links sent for', selectedInvoices);
    } else if (action === 'send_reminder') {
      selectedInvoices.forEach(async (id) => {
        // await XEPayService.sendReminder(id); // Not implemented yet
      });
      console.log('Reminders sent for', selectedInvoices);
    } else {
                    ...
                    <td key={colIndex} className="px-6 py-4 text-sm text-gray-900">
                      {column.key === 'actions' ? (
                        <div className="flex items-center space-x-2">
                          ...
                          <Button variant="ghost" size="sm" icon={Trash2} onClick={() => onDeleteInvoice?.(invoice)} className="text-red-600 hover:text-red-700" />
                ...
                {expanded === invoice.id && (
                  <tr>
                    <td colSpan={columns.length} className="px-6 py-4 bg-gray-50">
                      <PaymentActions invoiceId={invoice.id} total={invoice.total} />
                    </td>
                  </tr>
                )}
              </>
