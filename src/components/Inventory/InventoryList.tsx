...
const InventoryList: React.FC<InventoryListProps> = ({
  ...
  // Extract unique categories and suppliers from items
  ...
  const supplierOptions = [
    ...
    // Suppliers will be managed separately, so this filter might not be needed
  ];
  ...
  const filteredAndSortedItems = useMemo(() => {
    let filtered = items.filter(item => {
      ...
      const matchesSupplier = !supplierFilter; // Supplier filter not implemented yet
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'sku':
          ...
          break;
        case 'currentStock':
          aValue = a.currentStock ?? a.current_stock ?? a.quantity ?? 0;
          bValue = b.currentStock ?? b.current_stock ?? b.quantity ?? 0;
          break;
        case 'unitPrice':
  ...
  const getStockStatusColor = (item: InventoryItem) => {
    ...
    if (minStock > 0 && currentStock <= minStock) return 'bg-yellow-100 text-yellow-800';
    if (reorderPoint > 0 && currentStock <= reorderPoint) return 'bg-orange-100 text-orange-800';
  ...
  const getStockStatusText = (item: InventoryItem) => {
    ...
    if (reorderPoint > 0 && currentStock <= reorderPoint) return 'Low Stock';
    return 'In Stock';
  };
                ...
                <TableRow key={item.id} hover>
                  ...
                  <TableCell>
                    ...
                    <div className="text-xs text-gray-500">
                      Min: {item.minStock ?? item.min_stock ?? 0} | Max: {item.maxStock ?? item.max_stock ?? 0}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    ...
                    <div className="text-xs text-gray-500">
                      Cost: {formatCurrency(item.costPrice ?? item.cost_price ?? 0)}
                    </div>
                  </TableCell>
