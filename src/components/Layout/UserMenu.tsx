...
const UserMenu: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const currentAvatar = user?.avatar?.split('?')[0]; // Remove query params for comparison
    const prevAvatar = prevAvatarRef.current?.split('?')[0];
  ...
  const getAvatarUrl = (avatar: string | undefined | null): string => {
    ...
    return `${baseUrl}?t=${avatarCacheKey || Date.now()}`;
  };
  ...
  const handleLogout = async () => {
    setIsOpen(false);
    await logout();
    // Navigation will happen in logout function
  };
