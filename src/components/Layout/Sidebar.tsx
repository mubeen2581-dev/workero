...
const allNavigationItems: NavItem[] = [
  ...
  { name: 'Leads', href: '/leads', icon: Users, badge: 3, permission: 'crm.view' },
  { name: 'Quotes', href: '/quotes', icon: FileText, badge: 7, permission: 'quotes.view' },
  { name: 'Jobs', href: '/jobs', icon: Briefcase, badge: 12, permission: 'jobs.view' },
  { name: 'Scheduling', href: '/scheduling', icon: Calendar, permission: 'scheduling.view' },
  { name: 'Live Map', href: '/scheduling/live', icon: MapPin, permission: 'scheduling.view' },
  { name: 'Inventory', href: '/inventory', icon: Package, permission: 'inventory.view' },
  { name: 'Invoices', href: '/invoices', icon: Receipt, badge: 5, permission: 'invoices.view' },
  ...
  { name: 'WhatsApp CRM', href: '/communication', icon: MessageCircle, badge: 2, permission: 'communication.view' },
  { name: 'Compliance', href: '/compliance', icon: FileText, permission: 'compliance.view' },
];
...
const Sidebar: React.FC = () => {
  ...
  const navigation = React.useMemo(() => {
    ...
    return allNavigationItems.filter(item => {
      ...
      return permissions.some(perm => can(user.role as any, perm));
    });
  }, [user]);
        ...
        {navigation.map((item, index) => (
          ...
          >
            <NavLink
              to={item.href}
              onClick={() => setMobileMenuOpen(false)}
              end={item.href === '/' || item.href === '/scheduling'}
              className={({ isActive }) =>
